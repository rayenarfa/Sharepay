// Simple JavaScript version of the createInvoicePdfDoc function
import { jsPDF } from 'jspdf';

/**
 * Creates a PDF invoice document for an order
 * @param {Object} order - The order object with all details
 * @returns {jsPDF} The PDF document
 */
export function createInvoicePdfDoc(order) {
  console.log(`Creating invoice PDF for order: ${order.orderNumber}`);
  ...
  y += 6;
  doc.text(`${order.shippingAddress.city}, ${order.shippingAddress.state} ${order.shippingAddress.zipCode}`, detailsStartX, y);
  y += 6;
  ...
  // Handle date conversion from Firebase Timestamp or Date object
  let processedDate;
  if (order.date && typeof order.date.toDate === 'function') {
    processedDate = order.date.toDate();
  ...
  doc.text(`SharePay | ISET GAFSA | ${process.env.VITE_EMAIL_USER}`, pageWidth / 2, footerStartY, { align: 'center' });
